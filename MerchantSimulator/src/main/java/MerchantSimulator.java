...
public class MerchantSimulator {
    ...
    public ResponseModel createAccount(String name, String cvr) {
        //make json
    ...
    public String getAccount(String cvr) {
        //make json
        HttpResponse<String> r = null;
        try {
            r = Unirest.get(endpoint + "/merchant" + "/"+cvr).header("Content-Type", "application/json").asString();
        } catch (UnirestException e) {
            e.printStackTrace();
        }
        return r.getBody();
